...
$(function() {
	...
	function setGenderClass() {
		$('fieldset:not(#reg-step-gender)').removeClass('race-2 race-3').addClass('race-' + race);
		$('#wrapper').addClass((race == 2) ? 'as' : 'van');
		flip = $('#reg-form').append('<div id="flip-img" class="gender-' + race + gender + '"></div>');
	}
	 ...
	 * Выбираем пол
	 */
	$('label[for="gender-af"], label[for="gender-am"], label[for="gender-vm"], label[for="gender-vf"]').on('click', function() {
	...
	// если есть селектор языка - высталяем язык и подписываемся на его обновление
	if ($('#lang-selector').length) {
		...
		$('#lang-selector').on('change', function (e) {
		    ...
		    // $.cookie('force_lang', this.value);
		    if (/cmd=/.test(location.search)) {
				location.href = location.href;				
		    } else {
		    	location.href = '/reg.pl?cmd=reg';	
		    }
		});
	}
	...
	function showCustomErrors(errors) {
		if (typeof errors == 'string') {
			...
			if (errors.social) {
				$('label.error[for="email"]').html(errors.social).css('display', 'block');
  ...
  var deviceCredentials = ExGodsMobile.accounts.getDeviceAccountCredentials();
  var userData = {};

	var formOptions = {
		dataType: 'json',
		
    url: ExGodsMobile.host_url + '/reg.pl',
    ...
    data: {
      ...
      formData.forEach(function(item) {
		...
		success: function(json) {
			...
			if (!json.error) {
			   ExGodsMobile.accounts.loginBy(ExGodsMobile.accounts.add("device", deviceCredentials, userData));
			} else {
	...
	regForm.validate({
		...
		rules: {
			title: {
				...
				maxlength: 20,
				remote: {
					url: ExGodsMobile.host_url + '/reg.pl?cmd=check_name',
					type: 'post',
					data: {
						name: function() {
							return $("#name").val();
